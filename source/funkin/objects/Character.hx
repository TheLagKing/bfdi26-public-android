	...
	public function new(x:Float, y:Float, ?character:String = 'bf', ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			default:
				...
				//trace(path);
				if (!FileSystem.exists(path) && !Assets.exists(path) && !Assets.exists(Paths.getSharedPath(characterPath)))
				{
				...
				try
				{
					...
					if (FileSystem.exists(path)){
						...
						jsonFile = Json.parse(File.getContent(path));
